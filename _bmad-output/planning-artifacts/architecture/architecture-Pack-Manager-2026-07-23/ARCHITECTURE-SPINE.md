...
- **Rule:** `Esc` never touches membership. Its clear-selection rung is **deleted,
  not re-pointed** — the cascade drops from three rungs to two and keeps
  close-dialog (`prd.md` FR-6). That is what keeps one `Esc` from mass-writing
  tombstones and poisoning a draft against `Update Everything`, the failure this
  rule's absence made reachable. There is no surviving second rung: `prd.md` FR-6
  called it close-drawer and the drawer retires with AD-17, so the cascade is
  close-dialog alone. AD-17 fixes that and `⌘L`'s sink; `prd.md` RP-2's parenthetical
  "(toggle the activity surface)" describes the retiring behaviour and is recorded as
  a PRD divergence.
...
| `epics.md` residuals for the next `bmad-correct-course` run | **OPEN — record only; do not edit `epics.md` here** | **The five residuals this row previously listed are all CLEARED**, by commit `5972109` "Clear the epics.md residual pile (#36)", which touched only `epics.md` and landed after the revision-9 spine was written. Verified item by item against the committed file this revision, not against the report of it: UX-PB.3d now *states* AD-25's snapshot rule in criterion prose ("a verification refresh that errors or times out never replaces or clears the snapshot it failed to refresh"); AD-21's substance reaches UX-PB.5b's criteria ("`skipUpgradePlanConfirmation` is plan-inert (AD-21) … writing it never advances the canonical revision"); the register's design-token row reads `CLOSED` — D35 with "Nothing blocked"; the `notarytool minos 15.0` question reads CLOSED by D34; `grep -c AD-27 epics.md` returns 32; and `grep -c 'ARCHITECTURE-SPINE.md[:#]*[0-9]'` returns **0**, so the line-number citations this row warned about are gone rather than merely repointed. **What remains is what revision 10 itself created, and the first three are the urgent class because they now contradict rules above.** **Story 3.5's criteria are written on the selection model AD-28 abolishes** — toggle, shift-range, tri-state, `⌘A`, Space, `⌘`-click, Clear and `Esc` against "the exact selectable identities", plus a `Clear` action and an `Esc` rung AD-28 deletes; restate them in membership terms, batched. (`epics.md` never names `⌘U` — `grep -c` returns 0 — so only `docs/SPEC.md` §4.11 carries that limb.) **`epics.md` FR-19 and NFR-6 still carry the D37-removed keyboard/VoiceOver and announcement obligations**, and so does Story UX-PB.1d — which D37 says explicitly **is not to be deleted**, because its pointer-hover ineligibility explanation is exactly what D37 protects; only its keyboard and VoiceOver limbs are in scope. UX-PB.3d's "renders and **announces** Results" is the same class. **UX-PB.1d already carries AD-16's not-native-`disabled` rule, and that is exactly the trap** — it reads "it uses `aria-disabled=\"true\"` rather than native `disabled`, keeps focus, announces its persistent reason … stays inert on activation", but the whole criterion is gated on a **`Given`/`When` about a keyboard or VoiceOver user**. D37 removes that trigger, so a run that strips the keyboard and VoiceOver limbs would delete the only stated trigger for the one rule AD-16 now *requires*. Re-gate it on pointer interaction; do not delete it. Its story title ("with keyboard, pointer, and VoiceOver explanation") needs the same treatment, and D37 says UX-PB.1d is not to be deleted. **UX-PB.2c's admission record still lists "result/verification state"**, which AD-29 deliberately moves to the terminal record; the story should carry that override the way UX-PB.5b now carries AD-22's. UX-PB.3d's "an attempt accumulates several append-only records" should read *two*, and its "no rule for which record is authoritative" is now answered by AD-29 rather than left as a known gap. **AD-18 and AD-29 are cited by too few of the stories they bind.** Measured, not estimated: AD-18 appears in `epics.md` at seven lines, of which the story-block hits are UX-PB.2b, UX-PB.3d, UX-PB.4a, UX-PB.4e and Story 6.5 — so the stories AD-18 *binds* and that cite it nowhere are **UX-PB.2c, UX-PB.2d, UX-PB.2f and UX-PB.4b**. AD-29 is cited nowhere at all (`grep -c` → 0), and it additionally binds UX-PB.2e and UX-PB.3g because each can drive an attempt terminal. **AD-28 is cited nowhere**, because this revision created it. **UX-PB.2e and UX-PB.3g still move running work to a `Cancelling` state** that `prd.md` FR-13 forbids by name and AD-16 now refuses — UX-PB.2e "running work moves to `Cancelling` and escalates through the existing process-group mechanics", UX-PB.3g "changes still-running Operations bound to that `planAttemptId` to `Cancelling`". Those are the only two occurrences in `epics.md`; an earlier draft of this row named UX-PB.2f and UX-PB.4c, which contain none, and a run following it literally would have opened two clean stories and left both defective ones. **This item is ordering-critical:** UX-PB.2e ships in wave 2, well before any later cleanup, and adding a `Cancelling` variant is an atomic AD-3 change across the Rust enum, `src/lib/ipc/types.ts`, the guards and the committed fixtures — so it must be corrected *before* UX-PB.2e is built, not after, or the correct-course run is undoing shipped wire surface instead of editing story text. `EXPERIENCE.md` carries the state too, so it needs the `bmad-ux` Update. **Stories 3.4 and 6.5 must gain AD-27** — this revision widened its `Binds` to them, and `epics.md` still reads "Governing invariants: AD-4, AD-5, AD-19" for Story 3.4 and "AD-3, AD-4, AD-5, AD-16, AD-18, AD-26" for Story 6.5. The `grep -c AD-27` → 32 above is **clearance evidence for the old list and the wrong measure for the new one**: it was already true before the widening, so it reads as done while the two newly bound stories are exactly the two that do not read the rule. **Two `epics.md` passages now assert the opposite of D36** — its DR-2 restatement and its Implementation-Entry register row both say automated 4.5:1 contrast "does not exist" and name it as the outstanding obligation on whichever story adds it. Commit `a201fb0` landed it. A builder reading either schedules shipped work, which is the exact error AD-1's second rule forbids, and the register is the more dangerous of the two because it is what a builder consults to decide what is startable. **Story UX-PB.5d is built almost entirely on D37-removed scope** — "As a keyboard and VoiceOver user at high zoom…", a keyboard/VoiceOver `When`, and a focus-order-and-announcements `Then` that `prd.md` names among the dropped obligations. Its zoom half survives under NFR-3, so restate it as UX-PB.1d is restated; do not delete it. Same class, also unnamed until now: **UX-PB.3a's** "the status channel announces plan start", which AD-17 has made optional rather than required, and `epics.md`'s surviving claim that "one manual VoiceOver pass joins `docs/RELEASE-CHECKLIST.md`" — D37 deleted that pass. **Story 3.2's pinned-row criterion still says `disabled`** ("pinned rows stay inert … and are explained, disabled, and excluded from every plan"). AD-16 binds Story 3.2 and now forbids the native state, so it needs the same correction as UX-PB.1d — it renders the same control. **UX-PB.2c's persist-failure criterion may read against AD-29** — "the prior consistent state is preserved rather than proceeding as if durably recorded" is readable as *do not admit*, which AD-29 forbids since the append gates nothing, and "no partial attempt record is left behind" is readable as deleting a line from an append-only file. Both readings recover in the author's favour, so this is a wording clarification rather than a defect. **Story 3.5 needs a criterion for the non-list views** — AD-28's native-default rule binds the accelerator map, but Story 3.5 is the only story naming `⌘A` and its `Given` is a row list, so the Dashboard, History and Settings appear in no criterion and the shipping `preventDefault` defect has no owner. **Story 3.2's Dependencies should name UX-PB.1d** — UX-PB.1d says "Blocks: Story 3.2" while Story 3.2 lists only "Story 3.1; deterministic plan-builder and UI fixtures", so the edge exists in one direction and nothing sequences the inertness contract even after the wording fix lands. **UX-PB.2d's "where applicable"** is undefined for verification refreshes — one has no `opId` until it becomes an Operation, so UX-PB.2d and UX-PB.3d can read it oppositely; correlation degrades, nothing wedges. Scope that run **by named heading, not by mention count**: the counts in `docs/DECISIONS.md` D37 were copied rather than measured and are wrong (re-measured in the PRD addendum). Nor by *this* spine's keyword count, which **rose** in revision 10 — applying D37 means recording what was retired, and recording it uses the words. Some of the matches were always material D37 protects by name, which is why the count was never the scope. Cite by `AD` id and subject, never by rule ordinal and never by line number. |
...
| Maintainer edits this spine cannot make | **OPEN — record only; workflow-unowned files** | The spine routes workflow-owned divergences to `bmad-correct-course` and `bmad-ux`. These are in files no workflow owns, so they are listed rather than fixed. **`docs/DECISIONS.md`:** D15's mechanism clause ("Disabled checkbox + tooltip with the `brew unpin` command") is overridden by AD-16's inertness rule and nothing records the supersession — only a decision can supersede a decision. D37's "Not yet applied" counts were copied rather than measured and are wrong in all three columns; revision 10 **is** the `bmad-architecture` Update D37 names there, so the spine's limb is now discharged. D36's title claims "D35's on-fill tokens get consumers" plural, while `--color-on-success` still has none — all three fixed sites use `text-on-accent`. **`docs/RELEASE-CHECKLIST.md`:** step 5 tells the verifier that row checkboxes and Manager headers "stage into the draft plan", which is D27 target state — at `HEAD` a row checkbox writes `selection` and a row action executes, so the step describes something that cannot be verified yet. It also validates no application accelerator, though `prd.md` §4.6 makes it RP-2's validation route. **`prd.md` itself:** RP-2 describes `⌘L` as "(toggle the activity surface)", the retiring `ActivityDrawer` behaviour — AD-17 now makes `⌘L` a focus jump to the sidecar region, so the parenthetical is stale. **Drafts for all of these are in `MAINTAINER-EDITS-2026-07-25.md`.** **`docs/SPEC.md`:** F5 was never added to §0.1's supersession list — the omission that let the membership conflict survive into a spine Open row — and §4.11 still lists `⌘U` and an `Esc` clear-selection rung that AD-28 removes. |
