...
- Rust IPC structs/enums use camelCase Serde wire names and mirror `src/lib/ipc/types.ts` one-for-one. A skipped Rust `Option<T>` maps to an optional TypeScript field; a serialized option maps to `T | null`.
...
- Do not hold Rust synchronization guards across `.await`, and do not use `unwrap()` on fallible user-reachable paths.
...
- Keep command payloads wrapped as `{ args: ... }`. Any IPC change must update Rust models, TypeScript mirrors/guards, and `dev/fixtures/ipc/*.json` together.
- Subscribe to native events before `get_state` hydration so startup detection cannot be missed. Clean up every listener; preserve the `Promise.allSettled` rollback behavior for partial subscription failure.
- Use narrow Zustand selectors in components and `Store.getState()` outside React. Replace objects/Sets immutably; put cross-store derived state in `src/store/index.ts` instead of duplicating it.
...
- Hooks must precede early returns. Effects must clean up subscriptions/timers and declare correct dependencies.
...
- Keep parser tests grounded in committed captures under `dev/fixtures/`. Never invent an unlabeled fixture; synthetic inputs require `_synthetic` plus their source and retirement condition.
...
- Read the relevant `docs/SPEC.md` contract and `docs/DECISIONS.md` entries before changing behavior. Use `docs/index.md` for navigation; treat `docs/IMPL_PLAN.md` as potentially stale history.
...
- A releasable commit reaching `main` can automatically merge the release PR and publish signed/notarized artifacts. Keep work off `main` until it is ready to ship.
...
- Nothing bulk-executes before the user sees the exact commands. Execution must re-derive adapter commands rather than trust arbitrary frontend command text.
- Every operation declares one atomic lock set. All Homebrew work takes the Brew lock; routed operations lock executor and subject; mise-managed npm/uv work also protects Mise. Preserve global concurrency, fairness, and refresh coalescing.
- Never run shell command strings. Spawn resolved absolute executables with argv, `env_clear`, an explicit environment, null stdin, and a new process group. No sudo or password prompt path is allowed.
...
- Keep operations reconstructible: status/output events, transcript, structured log, and crash journal share the operation ID. On shutdown, cancel and reap process groups; never signal journaled PIDs after restart because PID reuse is unsafe.
- Settings and journal rewrites remain atomic. Journal/log failures must not fail package operations. Diagnostics must reject symlinks both when selecting and when streaming files.
...
- Selection must continue excluding pinned packages and greedy casks. Unknown latest versions remain `null`; never fabricate a version delta.
- The current production surface is 20 commands and six events. Ignore older 17-command/five-event references. `DECISIONS.md` D23a supersedes D23 for live-verified mas behavior, and D25 supersedes D20 for signed/notarized in-app updates.
