...
3. **Bulk work is previewed exactly.** A pure plan contains the commands, exclusions, locks, notes, and warnings shown before execution.
...
7. **Operations are reconstructible.** Status events, output events, transcripts, structured logs, and the crash journal share operation IDs.
