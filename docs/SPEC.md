...
Status: FINAL, amended 2026-07-24. Synthesized from three design candidates and three judge reports, then superseded in the update-experience areas named in §0.1 by the approved Correct Course proposal and Decisions D27-D30. All fixture claims below were verified against the files in `/Users/sallvain/Projects/Pack-Manager/dev/fixtures/` on 2026-07-22.
...
6. **Nothing runs that was not staged and shown.** Every Package and Manager update enters the persistent Upgrade Plan. Rust previews exact commands, rebuilds them before execution, and executes only a confirmed matching plan.
7. **Every confirmed plan attempt is reconstructible from disk.** The durable `planAttemptId` correlates reviewed intent, Operations, events, transcripts, journal records, verification, Results, and Retry lineage. The one-use preview `planId` remains a separate trust capability.

### 0.1 Superseding update-experience contract (2026-07-24)

This section supersedes older text in this document wherever the two conflict:

- Package-row, Manager-header, Manager-wide, and `Update Everything` actions
  add eligible items to one persistent editable Upgrade Plan. No update action
  executes immediately.
- The plan sidecar is hidden when empty, appears after the first item is added,
  and persists across Dashboard and Manager navigation.
- The final confirmation is a separate modal dialog. Its opt-out checkbox is
  shown only inside that dialog. The safe default remains confirmation enabled,
  and Settings can restore it.
- One confirmed plan attempt may be active at a time. Manager concurrency
  occurs inside that attempt through the existing lock-set scheduler.
- The running sidecar presents plan-level progress and then a Results summary.
  Success is terminal only after required verification refreshes complete.
- Activity is a first-class destination for the active attempt and detailed
  replay. History contains one immutable entry per confirmed plan attempt, with
  Operation evidence nested inside it.
- `Interaction required` is reserved for a trusted closed Manager-specific
  classifier or explicit native signal. Unrecognized silence follows the
  ordinary stall flow.
...
Columns: checkbox, name, installed, latest (version-delta treatment §4.6), status badge, and plan-membership action. uv rows expand to show executables. Pinned brew formulae (JSON `pinned` field) show `Pinned`, are dimmed and non-selectable, and explain on attempted interaction: "This package is pinned and cannot be updated. Unpin it to update package." Current and excluded Packages are also non-selectable with a reason. Greedy-only casks (two-call diff, §5.4) remain excluded unless explicitly opted in. Versions render verbatim. Packages with updates appear first by default, with a filter that can show all Packages.
**Acceptance:** Package actions add only eligible canonical Package identities to the persistent Upgrade Plan; no row action executes. Pinned/current/excluded rows cannot be selected and expose a plain-language reason.

### F4 (P0) Persistent Upgrade Plan and final confirmation

Dashboard `Update Everything`, per-Manager update actions, Manager self-update actions, and Package actions edit one persistent Upgrade Plan. Its Manager sections show each staged Package as `name installed -> latest`, using warning color for the installed version and success color for the target. A bold underlined `Show Update Command` control reveals Rust-produced exact commands beneath the Package list. Each Manager self-update is independent plan membership; there is no global `includeSelfUpdates` toggle. Exclusions and stale warnings remain visible.

The plan's `Confirm N Updates` button opens `Proceed with Upgrade Plan?` by default. That dialog shows the exact commands and offers `Change Plan` and `Confirm N Updates`. Only the dialog contains `Disable upgrade plan command execution confirmation`, plus warning copy that Settings can restore the preference. If disabled, later confirmations still rebuild and authenticate the plan but skip only this modal.

Each returned preview `planId` remains a bounded one-use backend capability. Confirm rebuilds from current canonical intent and executes only if the preview still matches; stale changes return to review. Successful confirmation creates a durable `planAttemptId`.
**Acceptance:** every Package and Manager update is visible in the persistent plan before execution; shown commands are byte-equal to spawned commands; no unreviewed item runs; confirmation is enabled by default and reversible in Settings.

### F5 (P0) Multi-select upgrade

Checkbox per eligible outdated row, header tri-state over visible selectable rows, shift-click range, Cmd-click toggle, and Cmd+A select all visible. `Add N to Plan` immediately adds the checked canonical identities to the persistent plan and clears the transient selection. A row-level `Update Package` action immediately adds only that Package to the same plan. Neither action executes.
**Acceptance:** frontend evidence proves the draft contains exactly the selected eligible `PackageRef`s, persists across Manager changes, and requires the common confirmation path before execution.

### F6 (P0) Manager self-update with dynamic routing

Each Manager title area shows a short standardized description, executable path, installed version beside the name, and a Manager-status badge reading `NO UPDATES` or `UPDATE AVAILABLE`. When a Manager update exists, warning-colored installed and success-colored target versions appear beneath the badge. The Manager update action adds that explicit update intent to the persistent Upgrade Plan.

Routes remain dynamically derived: brew→ViaRefresh (`brew update`); mise→Routed(brew, `brew upgrade mise`); npm→InBand (`npm install -g npm@latest`); uv→Routed(mise, `mise upgrade uv`); rustup→InBand (`rustup self update`). Ownership/routing is presented near the Manager identity in plain language, not as unexplained bottom-right metadata. The npm reset warning remains at the point of action. Routed updates hold both Manager locks (§5.7).
**Acceptance:** route precedence remains fixture-tested; frontend evidence proves a Manager update becomes removable plan membership and does not call a direct self-update execution path.

### F7 (P0) Plan Activity, live streaming, cancellation, and stalls

Every Operation streams stdout/stderr line-batched (≤50ms/64 lines, 8KiB force-flush) into the active plan's Activity detail. The sidecar shows per-item progress in real time; a verified completed Package replaces `installed -> latest` with the single new version. Only one confirmed attempt may be active, while its Managers may run concurrently under the lock scheduler.

Primary cancellation is `Cancel plan`: queued Operations for that `planAttemptId` become `Skipped`, running process groups receive SIGTERM→5s→SIGKILL, and all outcomes remain visible. Stall defaults to 120s. A known prompt may become `Interaction required` only through a closed Manager-specific classifier or explicit native signal; otherwise silence follows the ordinary Keep waiting / Copy command / Cancel flow. Pack-Manager never supplies passwords.
**Acceptance:** scheduler and native-boundary evidence covers one-active-plan enforcement, attempt-wide cancellation, trusted prompt classification, ordinary unknown stalls, and durable terminal outcomes.

### F8 (P0) Plan-attempt History, Results, transcripts, and journal

Every Operation writes a transcript (§6) and crash-safe start/finish records. Every confirmed plan also persists one immutable attempt record containing reviewed scope, command snapshot, `planAttemptId`, Operation identities, verification results, terminal Results, and optional retry lineage. History lists one row per confirmed attempt; opening it replays the plan in Activity with Operation-level command and transcript detail. Interrupted attempts surface honestly on relaunch, and recorded pgids are never signaled on startup. Legacy uncorrelated records remain labeled legacy Operations.
**Acceptance:** plan-attempt round-trip and migration tests reconstruct multi-Manager attempts, preserve the first failure across Retry, and never fabricate plan grouping for legacy data.
...
Dark theme is the only MVP theme (tokens §4.1, structured for a future light theme). Icon: programmatic 1024px PNG via `dev/icon/generate_icon.py` (Pillow) piped into `npx tauri icon`; generated set committed under `src-tauri/icons/`. Release delivery is signed, notarized, and stapled as required by D25/D25a; stale ad-hoc-only language is superseded.

### F11 (P0) Settings

Persisted at `~/Library/Application Support/Pack-Manager/settings.json`: `runBrewUpdateOnRefresh` (default true), `autoRefreshOnLaunch` (true), `stallAfterSecs` (120), `upgradeHardCapMins` (30), `logLevel` ('debug' for own crate), `includeGreedyByDefault` (false), and `skipUpgradePlanConfirmation` (default false). A patch is persisted before in-memory settings and the canonical-state revision advance; failed persistence leaves both unchanged. Install threshold, operation hard cap, and application log level are editable. The obsolete `autoOpenDrawer` key may be read as inactive legacy input but is not an active preference. The view also provides the read-only Environment Report and maintenance actions.
...
2. **Delegated**: `managed_by` is another _detected_ manager → Routed to that executor (mise→`brew upgrade mise`, uv→`mise upgrade uv`). Executor absent → fall through.
...
| uv                                                                                                                                                                                                                                                               | `uv tool list` (60s) → `uv tool list --outdated` (120s; empty stdout = clean; populated format fixture-tested)                                                                                                               | `uv tool upgrade <name…>`                                                                                                                          | —                                                                                      |
...
- **uv tool list**: `warning:` lines from either stream → `HealthIssue` (extract tool name between backticks independently of the optional reinstall suffix; the fixture line `warning: Tool \`aider-chat\` environment not found (run \`uv tool install aider-chat --reinstall\` to reinstall)`is runnable only because its suggestion exactly matches the allowlisted command derived from the safe tool name). Altered, missing, or malformed suggestions remain in`detail`but set neither`fixCommand`nor backend fix argv. Tool lines`^(\S+) v(\S+)$`; `- exe`lines accumulate into`meta.executables`(verified: 12 tools in each capture;`claude-code-tools`has 17 executables in`uv_tool_list.txt`; `serena-agent v1.6.2.dev0`proves non-semver versions).`--outdated`: empty output = clean; the populated 2026-07-23 uv 0.11.30 capture observes parent rows using `name vINSTALLED [latest: LATEST]`followed by the same`- exe`child rows. Parse only valid parent rows into the outdated overlay, strip uv's structural`v`from the installed version, capture`LATEST`, and ignore child rows because executable metadata belongs to the inventory. Unknown but valid parent suffixes degrade to `latest: null`; unmatched lines are ignored so the UI receives neither bogus packages nor a fabricated delta.
...
interface DetectionReport {
  ...
  env: {
    path: string;
    entries: string[];
    source: "loginShell" | "staticFallback" | "merged";
    home: string;
  };
}
interface ManagerInfo {
  ...
  installHint?: string; // absent: e.g. "brew install mas"
}
...
interface Package {
  ...
  latest: string | null; // verbatim; null = unknown (no fabricated deltas)
...
| WARN                                                                                                                                                                                               | login-shell probe failure (StaticFallback), stall fired, ExpectedNonZero exits, health issues found, BrewLockBusy, event-buffer overflow, frontend errors (target `pack_manager_lib::frontend`)                                                                                                                                                                              |
